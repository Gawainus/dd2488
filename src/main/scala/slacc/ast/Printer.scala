...
object Printer {
  ...
  def getExprString(exprDecl : ExprTree, tabCount : Int) : String = exprDecl match {
      case And(_,_) => "(" + getExprString(exprDecl.asInstanceOf[And].lhs, tabCount) + " && " + getExprString(exprDecl.asInstanceOf[And].rhs, tabCount) + ")"
      case Or(_,_) => "(" + getExprString(exprDecl.asInstanceOf[Or].lhs, tabCount) + " || " + getExprString(exprDecl.asInstanceOf[Or].rhs, tabCount) + ")"
      case Plus(_,_) => "(" + getExprString(exprDecl.asInstanceOf[Plus].lhs, tabCount) + " + " + getExprString(exprDecl.asInstanceOf[Plus].rhs, tabCount) + ")"
      case Minus(_,_) => "(" + getExprString(exprDecl.asInstanceOf[Minus].lhs, tabCount) + " - " + getExprString(exprDecl.asInstanceOf[Minus].rhs, tabCount) + ")"
      case Times(_,_) => "(" + getExprString(exprDecl.asInstanceOf[Times].lhs, tabCount) + " * " + getExprString(exprDecl.asInstanceOf[Times].rhs, tabCount) + ")"
      case Div(_,_) => "(" + getExprString(exprDecl.asInstanceOf[Div].lhs, tabCount) + " / " + getExprString(exprDecl.asInstanceOf[Div].rhs, tabCount) + ")"
      case LessThan(_,_) => getExprString(exprDecl.asInstanceOf[LessThan].lhs, tabCount) + " < " + getExprString(exprDecl.asInstanceOf[LessThan].rhs, tabCount)
      case Equals(_,_) => getExprString(exprDecl.asInstanceOf[Equals].lhs, tabCount) + " == " + getExprString(exprDecl.asInstanceOf[Equals].rhs, tabCount)
      case ArrayRead(_,_) => getExprString(exprDecl.asInstanceOf[ArrayRead].arr, tabCount) + " [" + getExprString(exprDecl.asInstanceOf[ArrayRead].index, tabCount) + "] "
      case ArrayLength(_) => getExprString(exprDecl.asInstanceOf[ArrayLength].arr, tabCount) + ".length"
      case MethodCall(_,_,_) => {
        var methodNoParams = getExprString(exprDecl.asInstanceOf[MethodCall].obj, tabCount)  + "." + getExprString(exprDecl.asInstanceOf[MethodCall].meth, tabCount) + "("
        var params = exprDecl.asInstanceOf[MethodCall].args
      ...
      case Self() => "self"
      case NewIntArray(_) => "new Int[" + getExprString(exprDecl.asInstanceOf[NewIntArray].size, tabCount) + "]"
      ...
      case If(_,_,_) => {
        ...
        sb.append(getNumTabs(tabCount + 1) + getExprString(exprDecl.asInstanceOf[If].thn, tabCount + 1) + "\n")
        if (exprDecl.asInstanceOf[If].els.isDefined) {
          sb.append(getNumTabs(tabCount) + "else \n " + getNumTabs(tabCount + 1) + getExprString(exprDecl.asInstanceOf[If].els.get, tabCount + 1))
        }
        sb.toString
      }
      case While(_,_) => {
        ...
        sb.append(getNumTabs(tabCount + 1) + getExprString(exprDecl.asInstanceOf[While].body, tabCount + 1) + "\n")
        sb.toString
      }
      case Println(_) => "println(" + getExprString(exprDecl.asInstanceOf[Println].expr, tabCount) + ")"
      case Assign(_,_) => exprDecl.asInstanceOf[Assign].id.value + " = " + getExprString(exprDecl.asInstanceOf[Assign].expr, tabCount)
      case ArrayAssign(_,_,_) => {
        val arr = exprDecl.asInstanceOf[ArrayAssign]
        arr.id.value + "[" + getExprString(arr.index, tabCount) + "] = " + getExprString(arr.expr, tabCount)
      }
